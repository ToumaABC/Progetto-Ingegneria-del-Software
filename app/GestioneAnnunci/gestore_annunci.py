...
# L'import di db globale non è più strettamente necessario per la sessione, 
# ma lo lasciamo se serve ad altre parti dei models.
...
class GestoreAnnunci:
    ...
    def eliminaAnnuncio(self, annuncio_id, user_id):
        annuncio = self.verifica_proprieta_annuncio(annuncio_id, user_id)
    ...
    def visualizza_annuncio(self, id_annuncio):
        ...
        # Istanziamento dei gestori con iniezione del db
        gestore_utente = GestoreUtente(self.db, mail)
        gestore_stanza = GestoreStanza(self.db)
        
        locatore = gestore_utente.visualizzaProfilo(annuncio.locatore_id)
        inquilini = gestore_stanza.visualizzaInquilini(id_annuncio)
        recensioni = gestore_stanza.visualizzaRecensioni(id_annuncio)
        media_voto = gestore_stanza.calcolaValutazioneMedia(id_annuncio)
    ...
    def verifica_proprieta_annuncio(self, annuncio_id, utente_id):
